 ...
 *
 * Input parameters:
 *   p    - Pointer to an initialized csv_parser structure that maintains parsing state
 *   s    - Pointer to the buffer containing CSV data to be parsed (can be NULL)
 *   len  - Size of the data buffer in bytes
 *   cb1  - Callback function called for each parsed field:
 *          void (*cb1)(void *field_data, size_t field_length, void *user_data)
 *          - field_data: pointer to field content (NULL if CSV_EMPTY_IS_NULL option set and field is empty)
 *          - field_length: length of the field in bytes
 *          - user_data: user-provided data pointer passed through
 *   cb2  - Callback function called at the end of each row:
 *          void (*cb2)(int terminator_char, void *user_data)
 *          - terminator_char: the character that terminated the row (CR, LF, or -1 for EOF)
 *          - user_data: user-provided data pointer passed through
 *   data - User-defined data pointer passed to callback functions
 *
 * Output values:
 *   Returns the number of bytes successfully parsed from the input buffer.
 *   If parsing stops due to memory allocation failure or parse error in strict mode,
 *   returns the position where parsing stopped. Check p->status for error conditions:
 *   - CSV_SUCCESS: parsing completed successfully
 *   - CSV_EPARSE: parse error occurred (only in strict mode)
 *   - CSV_ENOMEM: memory allocation failed
...
csv_parse(struct csv_parser *p, const void *s, size_t len, void (*cb1)(void *, size_t, void *), void (*cb2)(int c, void *), void *data)
{
  ...
  if (s == NULL) return 0; // Если входная строка пустая, сразу возвращаем 0

  unsigned const char *us = s;  // Преобразуем входные данные к массиву байт
  unsigned char c;              // Текущий обрабатываемый символ
  size_t pos = 0;               // Сколько символов обработано

  // Копируем ключевые поля структуры в локальные переменные для ускорения доступа
  ...
  // Если буфер ещё не выделен, выделяем память
  if (!p->entry_buf && pos < len) {
    if (csv_increase_buffer(p) != 0) { // Если не удалось выделить память — сохраняем состояние и возвращаем pos
      p->quoted = quoted, p->pstate = pstate, p->spaces = spaces, p->entry_pos = entry_pos;
      return pos;
    }
  }

  // Основной цикл по всем символам входной строки
  while (pos < len) {
    // Проверяем, хватает ли памяти для нового символа, если нет — увеличиваем буфер
    ...
    c = us[pos++]; // Получаем очередной символ

    // Обработка состояния парсера
    switch (pstate) {
      case ROW_NOT_BEGUN:
      case FIELD_NOT_BEGUN:
        // Пропускаем пробелы и табы вне поля
        if ((is_space ? is_space(c) : c == CSV_SPACE || c == CSV_TAB) && c!=delim) {
          continue;
        }
        // Если встретили символ конца строки (CR/LF)
        else if (is_term ? is_term(c) : c == CSV_CR || c == CSV_LF) {
          if (pstate == FIELD_NOT_BEGUN) {
            SUBMIT_FIELD(p); // Завершаем поле
            SUBMIT_ROW(p, c); // Завершаем строку
          } else {
            // Если строка ещё не начата, по опции можно вызвать SUBMIT_ROW
        ...
        // Если встретили разделитель (запятая)
        else if (c == delim) {
          SUBMIT_FIELD(p); // Завершаем поле
          break;
        }
        // Если встретили кавычку — начинаем поле в кавычках
        else if (c == quote) {
          pstate = FIELD_BEGUN;
          quoted = 1;
        }
        // Любой другой символ — начинаем поле, не в кавычках
        else {
          pstate = FIELD_BEGUN;
          quoted = 0;
          SUBMIT_CHAR(p, c); // Добавляем символ в буфер поля
        }
        break;
      ...
      case FIELD_BEGUN:
        ...
        if (c == quote) {
          if (quoted) {
            SUBMIT_CHAR(p, c); // Внутри кавычек — добавляем кавычку
            pstate = FIELD_MIGHT_HAVE_ENDED; // Возможно, поле закончилось
          } else {
            // Ошибка строгого режима: двойная кавычка вне кавычек
        ...
        // Если встретили разделитель
          ...
          if (quoted) {
            SUBMIT_CHAR(p, c); // Внутри кавычек — просто добавляем
          } else {
            SUBMIT_FIELD(p); // Вне кавычек — завершаем поле
          }
        }
        ...
        // Пробелы и табы вне кавычек
        else if (!quoted && (is_space? is_space(c) : c == CSV_SPACE || c == CSV_TAB)) {
          SUBMIT_CHAR(p, c);
          spaces++;
        }
        // Любой другой символ
      ...
      case FIELD_MIGHT_HAVE_ENDED:
        // После кавычки внутри кавычек — проверяем, чем поле закончилось
        if (c == delim) {
          entry_pos -= spaces + 1; // Убираем пробелы и кавычку
          SUBMIT_FIELD(p);
        }
        else if (is_term ? is_term(c) : c == CSV_CR || c == CSV_LF) {
          entry_pos -= spaces + 1;
          ...
          if (spaces) {
            // Ошибка строгого режима: неэкранированная двойная кавычка
            if (p->options & CSV_STRICT) {
              p->status = CSV_EPARSE;
              p->quoted = quoted, p->pstate = pstate, p->spaces = spaces, p->entry_pos = entry_pos;
              return pos-1;
            ...
            // Две кавычки подряд — продолжаем поле
            pstate = FIELD_BEGUN;
          }
        }
        else {
  ...
  // Сохраняем локальные переменные обратно в структуру
  p->quoted = quoted, p->pstate = pstate, p->spaces = spaces, p->entry_pos = entry_pos;
  return pos; // Возвращаем количество обработанных символов
}
